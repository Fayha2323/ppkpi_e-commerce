...
if (isset($_POST['simpan'])) {
    ...
    if (!in_array($ext, $ekstensi)) {
        ...
        $insert = mysqli_query($koneksi, "INSERT INTO barang (produk, harga, foto) VALUES ('$produk','$harga','$foto')");
        if (!$insert) {
            header("location:?pg=tambah_produk&pesan=tambah-gagal");
        } else {
...
if (isset($_POST['edit'])) {
    ...
    if (!in_array($ext, $ekstensi)) {
        ...
        move_uploaded_file($_FILES['foto']['tmp_name'], 'upload/' . $foto);
        $update = mysqli_query($koneksi, "UPDATE barang SET produk='$produk', harga='$harga', foto='$foto' WHERE id='$id'");
...
<form action="" method="post" enctype="multipart/form-data">
    <div class="mb-3">
        ...
        <input required value="<?php echo isset($_GET['edit']) ? $rowEdit['produk'] : '' ?>" type="text" class="form-control" placeholder="Masukkan Nama Barang" name="produk">
    </div>
